...
Future<void> _saveToGallery() async {
  ...
  try {
    ...
    final file = File('${dir.path}/status_${DateTime.now().millisecondsSinceEpoch}.png');
    await file.writeAsBytes(image);

    await SaverGallery.saveImage(image, name: 'status_${DateTime.now().millisecondsSinceEpoch}', androidRelativePath: 'Pictures/StatusHub',androidExistNotSave:true );
...
Future<void> _shareStatus() async {
  try {
    ...
    await Share.shareXFiles([XFile(file.path)], text: 'Made with Status Hub 🔥');
  } catch (e) {
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
